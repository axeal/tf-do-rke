...
data "template_file" "nodes-hardened" {
  template = file("files/nodes-hardened.yml.tmpl")
  vars = {
    nodes = chomp(
      join(
        "",
...
resource "local_file" "rke-config-hardened" {
  content  = data.template_file.nodes-hardened.rendered
  filename = "${path.module}/hardened-cluster.yml"
}
...
resource "null_resource" "rke-state" {
  provisioner "local-exec" {
    ...
    command = "rm -f *cluster.rkestate kube_config_*.yml"
  }
}
